String VERSAO = "V0704 - 21/03/2019";
//---------------------------------------
...
struct botao1 {
  ...
  char contador = 0;
  const char* modelo = "interruptor",*nomeInter = "Com1",*tipo = "0";
} botao1;
struct botao2 {
  int entrada = 25, rele = 26;
  boolean estado = 0, estado_atual = 0  , estado_antes = 0;
  char contador = 0;
  const char* modelo= "interruptor", *tipo= "0",*nomeInter = "Com2";
} botao2;
struct botao3 {
  int entrada = 14, rele = 27;
  boolean estado = 0, estado_atual = 0  , estado_antes = 0;
  char contador = 0;
  const char* tipo= "0", *modelo= "interruptor", *nomeInter = "Com3";
} botao3;
struct botao4 {
  int entrada = 12, rele = 13;
  boolean estado = 0, estado_atual = 0  , estado_antes = 0;
  char contador = 0;
  const char* tipo= "0",* modelo= "interruptor",* nomeInter = "Com4";
} botao4;
...
void loop() {
  ...
  pisca_led(LED_VERDE,true);
	/*
			ATUALIZANDO DATA E HORA PELO WIFI
	*/
	timeClient.update();
  ...
  { 
    ...
    gravaLog(" "+hora_ntp + " - Configurações da Central ", logtxt, 4);
    servidor      = root["servidor"];
    serv = String(servidor);
    gravaLog(" "+hora_ntp + "   Servidor Banco de dados:   "+String(servidor), logtxt, 4);

    botao1.nomeInter  = root["int_1"];    
    botao1.tipo     	= root["tipo_1"];     
    botao1.modelo   	= root["sinal_1"];  
    gravaLog(" "+hora_ntp + "   Interruptor 1 : "+String(botao1.nomeInter)+" / "+String(botao1.tipo)+" / "+String(botao1.modelo), logtxt, 4);

    botao2.nomeInter  = root["int_2"];    
    botao2.tipo     	= root["tipo_2"];     
    botao2.modelo   	= root["sinal_2"];  
    gravaLog(" "+hora_ntp + "   Interruptor 2 : "+String(botao2.nomeInter)+" / "+String(botao2.tipo)+" / "+String(botao2.modelo), logtxt, 4);

    botao3.nomeInter  = root["int_3"];    
    botao3.tipo     	= root["tipo_3"];     
    botao3.modelo   	= root["sinal_3"];  
    gravaLog(" "+hora_ntp + "   Interruptor 3 : "+String(botao3.nomeInter)+" / "+String(botao3.tipo)+" / "+String(botao3.modelo), logtxt, 4);

    botao4.nomeInter  = root["int_4"];    
    botao4.tipo     	= root["tipo_4"];     
    botao4.modelo     = root["sinal_4"];
    gravaLog(" "+hora_ntp + "   Interruptor 4 : "+String(botao4.nomeInter)+" / "+String(botao4.tipo)+" / "+String(botao4.modelo), logtxt, 4);
  ...
  */
  ...
  {
		gravaLog(" "+hora_ntp + " - Entrando em modo ACCESS POINT e apagando configurações WIFI ", logtxt, 1);
		/*
    ...
    */
    ...
    {
      gravaLog(" "+hora_ntp + " - ERRO 0102 - Falha ao conectar no WIFI modo ACCESS POINT", logtxt, 1);
      delay(2000);
      ESP.restart();
      delay(1000);
    }
  ...
  {
    gravaLog(" "+hora_ntp + "\n - DESLIGANDO TODOS OS RELES", logtxt, 4);
  ...
  */
  ...
  if (URL != "vazio") 
  {
    ...
    if (codidoExec == "00011")
    {
      gravaLog(" "+hora_ntp+" - Novo valor de leitura do sensor MQ-2: " + String(valorMQ_Novo), logtxt, 4);
      EEPROM.begin(64);
    ...
    if (requisicao == "00014")
    {
      ...
      SPIFFS.remove("/param.txt");
      criarArquivo();
      gravaLog(" "+hora_ntp+" - Configuração minima gravada na central", logtxt, 1);
